...
public class DepositStrategy implements TransactionStrategy {
    ...
    public Transaction execute(TransactionCommand command) {
        log.info("Deposit for user {} transaction {}",
        ...
        Transaction tx = transactionRepository.save(
                TransactionMapper.fromCommand(command, wallet,
                        TransactionType.DEPOSIT, balanceBefore, wallet.getBalance()));

        walletEventRepository.save(
                WalletEventFactory.depositCompleted(command, wallet, balanceBefore, null));

        log.info("Deposit completed for user {} transaction {}",
                command.getUserId(), command.getTransactionId());
        return tx;
    }
}

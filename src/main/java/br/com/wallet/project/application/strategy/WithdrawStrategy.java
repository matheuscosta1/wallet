...
public class WithdrawStrategy implements TransactionStrategy {
    ...
    public Transaction execute(TransactionCommand command) {
        log.info("Withdraw for user {} transaction {}",
                command.getUserId(), command.getTransactionId());

        Wallet wallet = walletUseCase.validateWallet(
                command.getUserId(), command.getTransactionId());
        ...
        Transaction tx = transactionRepository.save(
                TransactionMapper.fromCommand(command, wallet,
                        TransactionType.WITHDRAW, balanceBefore, wallet.getBalance()));

        walletEventRepository.save(
                WalletEventFactory.withdrawCompleted(command, wallet, balanceBefore, null));

        log.info("Withdraw completed for user {} transaction {}",
                command.getUserId(), command.getTransactionId());
        return tx;
    }
}
